...
var _apiKey = null;
var _rateLimiter = new RateLimiter(config.defaultLimitString);
...
module.exports = {
  ...
  setRateLimit: (limitString) => _rateLimiter = new RateLimiter(limitString),
...
function schedule(options, callback) {
  if (options.static) {
    ...
    _rateLimiter.schedule( function() {
      _get(options, callback)
    });
  }
};
