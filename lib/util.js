...
module.exports = function(platform){
  ...
  util.request = function (options, callback) {
    if (_useRedis) {
      ...
      if (options.static) {
        ...
        util.schedule(function (done) {
          util._get(options, function () {
            if (done) { // always function
              done(); // if statement not needed
            }
            callback.apply(this, arguments);
          });
        });
  ...
  util.schedule = function (fn) {
    _limiterPer10s.schedule(function (done1) {	// calls fine and waits one second before calling the next
        _limiterPer10min.schedule(function (done2) {	// Causing calls to take 2x as long
          fn(function () {
            if (done1) {
              done1();
            }
