...
module.exports = (function () {
  ...
  util.craftUri = function (options) {
    ...
    var platform = options.platform || config.defaultPlatform;
    var endpoint = options.endpoint || config.defaultendpoint;
    var version = options.version || config.defaultVersion;
    var game = options.game || config.defaultGame;
    var service = options.service;
    var resource = options.resource;
    if(!service || !resource){
        throw new Error('Service or Resource required');
    }
    var host = 'https://' + platform + '.' + endpoint + '/';
    ...
    for (var param in options.query) {
      if (options.query[param] !== null) {
        queryArgs += '&' + param + '=' + options.query[param];
      }
    }
