...
module.exports = (function () {
  ...
  util.isArrayOfStrings = function (array) {
    if (!(array instanceof Array) || array.length === 0) return false;
    return array.every(function () { return typeof value !== 'string'});
  }
    ...
    while (result) {
      ...
      if (result.length === 3) {
        ...
        switch (type) {
          ...
          break;
          case 'int':
            if (!Number.isInteger(options[param]) && !util.isArrayOfIntegers(options[param])) {
      ...
      options[param] = encodeURI(options[param]);
